...
accuarcy_subtype <- cbind(accuarcy_subtype, rep('subtype',5))
...
accuarcy_stage <- rbind(c('model',0.8455),accuarcy_stage)

accuarcy_stage <- cbind(accuarcy_stage, rep('stage',5))
colnames(accuarcy_stage) <- c('source','accuarcy','class')

datatable <- rbind(accuarcy_subtype, accuarcy_stage)
datatable$accuarcy <- substr(datatable$accuarcy, 1,4)

datatable$accuarcy <- as.numeric(datatable$accuarcy)
